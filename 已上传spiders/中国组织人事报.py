...
class TestSpider(scrapy.Spider, ABC):
    name = 'a61998310eec9d9f785c643046f8388d'
    web_name = '中国组织人事报'
    custom_settings = {'LOG_FILE': './spiders/logs/zuzhirenshiSplog.txt'}
    url = 'https://www.zuzhirenshi.com/'
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) '
                      'Chrome/113.0.0.0 Safari/537.36 Edg/113.0.1774.576',
    }
    web_data = {'web_id': 'a61998310eec9d9f785c643046f8388d', 'web_name': '中国组织人事报'}
    count = 0
    data_lis = []
    ...
    def get_the_article_urls(self, response):
        article_url_id_code_lists = [i for i in re.findall('id":"(.*?)"columnId"', response.text)]
        for article_url_id in article_url_id_code_lists:
            url = "https://www.zuzhirenshi.com/api/welcome/selectNews?id=" + article_url_id.replace("\"", "").replace(
                ",", "")
