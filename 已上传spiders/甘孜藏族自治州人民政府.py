...
class TestSpider(scrapy.Spider, ABC):
    name = '256d0b07bc092c2324abbfc960ca9b77'
    web_name = '甘孜藏族自治州人民政府'
    custom_settings = {'LOG_FILE': './spiders/logs/甘孜藏族自治州人民政府log.txt'}
    url = 'http://www.gzz.gov.cn/'
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) '
                      'Chrome/113.0.0.0 Safari/537.36 Edg/113.0.1774.576',
    }
    web_data = {'web_id': '256d0b07bc092c2324abbfc960ca9b77', 'web_name': '甘孜藏族自治州人民政府'}
    count = 0
    data_lis = []
    ...
    def get_the_article_lists(self, response):
        ...
        url = "http://www.gzz.gov.cn/site-service/c/manuscript/getManuscriptListByColumnId"
        post_data = {"columnId": 3,
                     "current": 1,
                     "size": 20,
                     "type": 'true',
                     "orderByType": 0,
    ...
    def get_the_article_urls(self, response):
        ...
        for one_data in data_lists['data']['records']:
            url = "http://www.gzz.gov.cn/" + str(one_data['columnFolder']) + '/article/' + str(one_data['id'])
            yield scrapy.Request(url, callback=self.parse_detail, dont_filter=True)

    def parse_detail(self, response):
        title = ''.join(response.xpath('//*[@id="article-detail"]/div[1]/text()').extract())
        pubdate = ''.join(response.xpath('//*[@id="article-detail"]/div[2]/div[1]/span//text()').extract())
        content = ''.join(response.xpath('//*[@id="contentBox"]//text()').extract())
        item = {
            'title': title,  # 标题
