...
class TestSpider(scrapy.Spider, ABC):
    name = '71f94088c7ed83feb6f16450a1ed381c'
    web_name = '国家标准化管理委员会'
    custom_settings = {'LOG_FILE': './spiders/logs/国家标准化管理委员会log.txt',
    ...
    web_data = {'web_id': '71f94088c7ed83feb6f16450a1ed381c', 'web_name': '国家标准化管理委员会'}
    count = 0
    data_lis = []
    ...
    def get_category_lists(self, response):
        the_category_url_lists = ['zqyj/','gwyxx/','bzhyw/','bzhdt','tzgg','zqyj']
        for category_url_list in the_category_url_lists:
            category_url = 'https://www.sac.gov.cn/xw/' + category_url_list
    ...
    def get_the_article_lists(self, response):
        the_article_url_lists = response.xpath('//*[@id="ajax分页"]/div[1]/div/div[1]/a/@href').extract()
        for the_article_url_list in the_article_url_lists:
            article_url = 'https://www.sac.gov.cn' + the_article_url_list
            yield scrapy.Request(article_url, callback=self.parse_detail, dont_filter=True)

    def parse_detail(self, response):
        title = ''.join(response.xpath('//*[@id="barrierfree_container"]/div[4]/div[3]/div[1]/div/text()').extract())
        pubdate = ''.join(response.xpath('//*[@id="barrierfree_container"]/div[4]/div[3]/div[2]/span/text()').extract())
        content = ''.join(response.xpath('//*[@id="content1"]//text()').extract())
        item = {
            'title': title,  # 标题
